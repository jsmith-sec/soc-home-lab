...
- **Corroborating evidence?** The child command line used `-EncodedCommand` (base64),
  a common obfuscation technique for download-and-execute payloads. The parent binary
  running from a Temp directory is itself abnormal for Office.
...
## 4. Verdict
**True positive.** An Office process launched an encoded PowerShell command, which
matches the initial-access-to-execution behavior of a phishing macro. (In this lab
the parent was a renamed `cmd.exe` used to reproduce the exact telemetry a real Word
macro produces.)
