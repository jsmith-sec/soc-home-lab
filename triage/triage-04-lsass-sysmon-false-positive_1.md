...
**Summary:** A burst of about 104 Critical LSASS alerts fired in roughly one second, all
sourced from the endpoint's own `Sysmon64.exe` inspecting processes. Confirmed **false
positive** (a sensor feedback loop); the rule was tuned to exclude the monitoring agent.
...
rule's dumping-mask list (the memory-read masks listed in Report 03). In effect, the
sensor watching lsass trips the rule that watches lsass. Confirmed no attacker tooling
was present in the surge.
...
- **Tune the rule:** add `and not process.name : ("Sysmon64.exe" or "Sysmon.exe")` so
  ...
  monitoring agent by source. The complete rule after both iterations:
  ```
  event.code : "10" and winlog.event_data.GrantedAccess : ("0x1010" or "0x1410" or "0x1438" or "0x143a" or "0x1fffff") and not process.name : ("Sysmon64.exe" or "Sysmon.exe")
  ```
