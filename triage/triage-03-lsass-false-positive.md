...
**Summary:** An early, overly broad LSASS rule alerted on benign Elastic Agent
(`agentbeat.exe`) access to `lsass.exe` at the `0x1000` query mask, which cannot read
process memory. Confirmed **false positive**; the rule was tuned to alert only on
memory-read access masks.
...
| Access mask | Meaning | Dumping-capable? |
|---|---|---|
| `0x1000` | PROCESS_QUERY_LIMITED_INFORMATION (no memory read) | No, benign |
| `0x1010`, `0x1410`, `0x1438`, `0x143a`, `0x1fffff` | Include memory-read rights (`PROCESS_VM_READ`) | Yes, dumping-capable |
...
- **Tune the detection:** restrict the rule's `GrantedAccess` clause to the memory-read
  masks so routine `0x1000` / `0x1400` queries no longer alert:
  ```
  winlog.event_data.GrantedAccess : ("0x1010" or "0x1410" or "0x1438" or "0x143a" or "0x1fffff")
  ```
  This removed the noise while preserving detection of a real LSASS dump (see Triage
  Report 01).
