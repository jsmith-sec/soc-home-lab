...
**Summary:** LSASS memory dump on domain controller `DC01` via `procdump64.exe` using
the `0x1fffff` (PROCESS_ALL_ACCESS) mask, preceded by Windows Defender being disabled.
Confirmed **true positive**; escalated to IR for domain-wide credential reset.
...
- **Remediate:** given a domain controller is affected, force a domain-wide credential
  reset. Priority order: reset the **krbtgt account twice within ~1 hour** (invalidates
  any forged Kerberos tickets), then rotate domain-user and cached service-account
...
> **Lab vs production:** this was a controlled simulation, so scope was already known. A
> real DC-compromise incident would additionally require forensic/memory imaging of the
> host, a hunt for the dumped credentials being reused elsewhere, and containment
> coordinated with IR before any service is restored.
